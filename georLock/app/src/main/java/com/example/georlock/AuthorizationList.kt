...
//        setContentView(R.layout.activity_authorization_list)
        ...
        Thread() {
            ...
            runOnUiThread {
                ...
                binding.accesslist.adapter = ArrayAdapter<String>(this, R.layout.layout_list, list)
                // 정보 클릭시 회원정보 수정 페이지로 전환
                binding.accesslist.setOnItemClickListener { adapterView, view, position, l ->
        ...
        // 검색버튼
        binding.searchAccessButton.setOnClickListener {
            Thread() {
                var list: ArrayList<String> = onGetSearchAccess("${binding.searchAccess.text.toString()}")
                runOnUiThread {
                    Log.i("testLog", "loginclick : ${list}")
                    binding.accesslist.adapter = ArrayAdapter<String>(
